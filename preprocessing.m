...
%read in Data 
%"THE GREAT CHANGE"
read_dir = 'C:\MATLAB\data\TMaze\';   %There is your EEG raw data. Please note that you have to adjust the read in function for different file formats. Here the gui of EEGlab might help to identify the function that you need. Remember that everything you clicked in the GUI can be seen as code if you use the code "eegh"

%"THE GREAT CHANGE"
write_dir = 'C:\MATLAB\data\Results\';           %This is the directory 
...
%"THE GREAT CHANGE"
files = dir([read_dir '*TMaze.xdf']); % This is the file format of brain vision (.vhdr [vmrk,eeg]), if you have another file format, insert the corresponding file type
...
%How many EEG channels are there (excluding heart, skin conductance, but also excluding online ground and reference channel)
Number_of_EEG_electrodes_without_reference_and_ground = 62;
...
for VP = 1:COUNTPARTICIPANT  %FOR EVERY FILE
	...
	for checkfilesi = 1:size(checkfiles)	%for every file in there
		...
		if strcmp(strrep(filenames{VP},'.set',''),strrep(checkfilenames{checkfilesi},'.set','')) == 1 	%check whether the present file is in there
    ...
    EEG = pop_loadxdf(strcat(read_dir,filenames{VP}), 'streamtype', 'EEG', 'exclude_markerstreams', {});
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'gui','off'); 
    EEG = eeg_checkset( EEG );	%LOAD THE DATA, all channels, all timepoints		
    ...
    %"THE GREAT CHANGE"
    EEG = pop_select( EEG,'nochannel',{'VEOG' 'HEOG'});   		%State the channels that you want to "ignore" insert your channel names (heart because of error in workspace selection)
    EEG = eeg_checkset( EEG );
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'overwrite','on','gui','off'); 

    

    %select the RESTING DATA IN GENERAL in THE COSCIENCE DATA !


    %Fieser Notchfilter ! (50, 150, 250: incl. 3. und 5. Harmonische...)
    EEG = pop_eegfiltnew(EEG, 'locutoff',47.5,'hicutoff',52.5,'revfilt',1,'plotfreqz',0);
    EEG = pop_eegfiltnew(EEG, 'locutoff',97.5,'hicutoff',102.5,'revfilt',1,'plotfreqz',0); %Dritte Harmonische Schwingung ! Ist immer da ! OBACHT ! Egal welche Phase, bei Drehstrom...
    ...
    EEG=pop_chanedit(EEG, 'lookup','C:\MATLAB\Plugins\eeglab2023.1\plugins\dipfit\standard_BEM\elec\standard_1005.elc','append',Number_of_EEG_electrodes_without_reference_and_ground,'changefield',{Number_of_EEG_electrodes_without_reference_and_ground+1 'labels' 'FCz'},'lookup','C:\MATLAB\Plugins\eeglab2023.1\plugins\dipfit\standard_BEM\elec\standard_1005.elc');
    ...
    EEG = pop_reref( EEG, [],'refloc',struct('labels',{'FCz'},'type',{''},'theta',{0.7867},'radius',{0.095376},'X',{27.39},'Y',{-0.3761},'Z',{88.668},'sph_theta',{-0.7867},'sph_phi',{72.8323},'sph_radius',{92.8028},'urchan',{63},'ref',{''},'datachan',{0}));
    ...
    Percentage_rejected_ICs(VP,1) = 1-(size(ICs_to_keep,2)/size(EEG.reject.gcompreject,2));
    ...
    %EEG = pop_loadset('filename',filenames{VP},'filepath',load_dir)
    EEG = pop_loadset('filename',strrep(filenames{VP},'.xdf','.set'),'filepath',load_dir)
    EEG = eeg_checkset( EEG );
    %segment the data once again as before
    
   
	%Segmentation of small segments with 6 second lenght (overlapping and dropping the other data stuff)
    seperate_script_for_first_segmentation
	% EEG = pop_epoch( EEG, {'S_T3_conflict_0' 'S_T3_ambiguous_1' 'S_T3_approach_1' 'S_T3_ambiguous_0' 'S_T3_approach_0' 'S_T3_ambiguous_3' 'S_T3_avoidance_1' 'S_T3_ambiguous_2' 'S_T3_conflict_1' 'S_T3_avoidance_0'}, [-1  7], 'newname', strcat(filenames{VP},'_intchan_avg_filt epochs2'), 'epochinfo', 'yes'); %selection: -1 to 5 seconds
    ...
    EEG.data = [];
    EEG.icaact = [];
    save('montage_for_topoplot_CSD_CoScience.mat','EEG')
    ...
    %EEG.data = [];
    %EEG.icaact = [];
    %save('montage_for_topoplot_Replikation_Hewig_2011_2022_MA.mat','EEG')
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%END OF STEP 8c
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    %clear all stuff that is not needed for next person
	STUDY = []; CURRENTSTUDY = 0; ALLEEG = []; EEG=[]; CURRENTSET=[];
    clear ICA_stuff1 ICA_stuff2 ICA_stuff3 ICA_stuff4 reject1 reject2 data backupEEGdata backupEEGICA proj variancekeep index ICA_act ICA_winv ICs_to_keep reject1 classifyICs m1 m2 m3 m4 m5 m6 event_back
	close all
end
...
save('Evaluation.mat','Percentage_channels_kept','Percentage_rejected_ICs','Percentage_variance_kept','filenames','Bad_channel_array','Number_Great_epochs_rejected','Number_of_EEG_electrodes_without_reference_and_ground','Channelz_value_automatic_detection')
